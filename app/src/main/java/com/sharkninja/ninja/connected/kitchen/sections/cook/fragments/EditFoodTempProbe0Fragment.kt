...
class EditFoodTempProbe0Fragment : DialogFragment() {
    ...
    private fun initCustomDial() {
        //get initial list
        homeViewModel.getFoodListProbe(homeViewModel.probe0FoodPreset.value.food)
    ...
    private fun snapToSelectedTemperature() {
        val currentTemp = if (homeViewModel.isProbeSet(PROBE_0)) {
            homeViewModel.preCookDashboardUiState.value.probe0FoodTempValue
        } else {
            if(isManualSelected()) {
    ...
    private fun onSave() {
        ...
        if (foodItem?.presetIndex != null) {
            homeViewModel.selectFoodProbe0(binding.foodSelectorMenu.foodSelection)
            homeViewModel.saveProbe0Selection(foodItem)
            homeViewModel.updateIsProbe0ToggleStatus(true)
            dismiss()
        }
    }
    ...
    private fun checkForChanges(): Boolean {
        val savedProtein = homeViewModel.probe0FoodPreset.value.food
        val savedTemperature = homeViewModel.preCookDashboardUiState.value.probe0FoodTempValue
    ...
    private fun showGoBackDialog() {
        ...
        }.show(parentFragmentManager, EDIT_FOOD_TEMP_PROBE_0_TAG)
    }

    companion object {
        private const val EDIT_FOOD_TEMP_PROBE_0_TAG = "editFoodTempProbe0Fragment"
    }
