...
class CookEditFoodTempProbe0Fragment : DialogFragment() {
    ...
    private fun snapToSelectedTemperature() {
        val desiredTemp = homeViewModel.selectedGrillState.value.probe1.desiredTemp
    ...
    private fun subscribeToVm() {
        ...
        homeViewModel.selectedGrillState.onEach {
            setSaveButtonEnabled(it.probe1.pluggedIn)
        }.flowWithLifecycle(lifecycle).launchIn(lifecycleScope)
    }
    ...
    private fun setSaveButtonEnabled(isPluggedIn: Boolean) {
        ...
        if(isButtonEnabled) {
            ...
            binding.saveButton.text = getString(R.string.save_changes_btn_cook_edit_probe_one_disabled)
        }
    }

    private fun initClickListeners() {
        binding.cookToolbar.setEditScreenToolbarTitle(getString(R.string.probe0_manual_title))
    ...
    private fun getFoodList() {
        val currentFood = homeViewModel.selectedGrillState.value.probe1.food.protein
    ...
    private fun onSave() {
                    val selectedTemperature = adapter.internalTempList.firstOrNull { temp -> temp.isSelected }
                    if (selectedTemperature != null) {
                        homeViewModel.editFirstThermometerSettings(
    ...
    private fun showGoBackDialog() {
        ...
        }.show(parentFragmentManager, COOK_EDIT_PROBE0_TAG)
    }

    private fun checkForChanges(): Boolean {
        var currentDesiredTemp = homeViewModel.selectedGrillState.value.probe1.desiredTemp
        if(currentDesiredTemp == 0) currentDesiredTemp = 100
        var currentProtein = homeViewModel.selectedGrillState.value.probe1.food.protein
    ...
    companion object {
        const val COOK_EDIT_PROBE0_TAG = "FRAGMENT_COOK_EDIT_PROBE0"
    }
